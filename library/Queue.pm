...
sub program_run {
    ...
    my ($script, $program, $input_path, $sample, $sh_path, $output_path, $threads, $config, $indate) = @_;
#    $input_path = sprintf ("%s/%s", $input_path, $sample);
    ...
    my $cmd = "perl $script -p $program -i $input_path -S $sample -l $sh_path -o $output_path -t $threads -c $config -d $indate";
#    print $cmd."\n";
    return $cmd;
}
1;
